...
agent_communication:
  ...
  - agent: "main"
    message: "v3.7.14 / build 54 — Session CORRECTION TESTFLIGHT RÉELLE (frontend-only, no backend changes). DELIVERED: (A) Versions bumped to 3.7.14 / iOS buildNumber 54 / Android versionCode 54. (B) **🔴 CRITICAL FIX — Voice IA paywall sur Pro provisoire**: src/services/featureFlags.ts `useIsPremium()` lisait UNIQUEMENT `useStore.user?.isPro` (legacy/demo) et ignorait `usePremiumStore` — donc après achat Apple TestFlight, le Pro provisoire 48h v3.7.11 ne débloquait PAS Voice IA / OCR Pro / Cloud features. **FIX**: useIsPremium subscribe maintenant explicitement à `usePremiumStore.isPro`, `trialEndsAt`, `provisionalProUntil` + legacy → retourne true si Pro confirmé OU essai trial actif OU **Pro provisoire actif** OU legacy. Résultat: 'Essai gratuit 7 jours' débloque IMMÉDIATEMENT toutes les features premium (Voice, OCR, Cloud, etc.) dès l'instant où la `purchase()` grant provisional Pro. (C) **🔴 CRITICAL FIX — Import facture 'unsupported image format gpt-4o-mini'**: email-import.tsx envoyait l'image brute en base64 ; iPhone shoots HEIC par défaut → litellm gpt-4o-mini rejetait avec 'unsupported_image_format'. **FIX**: les 3 chemins (DocumentPicker, Photo, Camera) convergent maintenant vers `parseImageFile()` qui appelle TOUJOURS `normalizeImageForUpload()` (HEIC/HEIF → JPEG via expo-image-manipulator) avant l'envoi. PDF détecté → envoyé avec mime 'application/pdf' (backend peut OCR la 1ère page). URI original conservé dans `_originalUri` pour preview ultérieur. (D) **NEW src/lib/errorSanitizer.ts** (~110 lignes) — `humanizeError(err)` détecte tous les patterns techniques ('unsupported image format', 'litellm', 'openai', 'HTTP 404/500', 'JSON Parse error', 'aborted', 'timeout', 'HEIC', 'unauthorized') et retourne {title, message, fallback?} human-friendly. Plus jamais 'gpt-4o-mini' ou 'litellm.BadRequestError' visible côté utilisateur. Toutes les Alert d'email-import passent maintenant par humanizeError → messages premium ('Format non reconnu / Le PDF original a été conservé', 'Service IA momentanément indisponible / Réessayez ou saisir manuellement'). (E) **Mon Classeur — Import IA banner supprimé**: dans documents.tsx le bandeau 'Import IA' (sparkles) au-dessus de la recherche était redondant avec les quick-action buttons Scanner/Importer PDF + empty state premium → supprimé. (F) **Tickets & Reçus — Import IA banner supprimé**: dans receipts.tsx, idem suppression. Les CTA premium Scanner + Ajouter manuellement restent. (G) **🔴 Force ticket type from Tickets & Reçus**: receipts.tsx 'Scanner' route avec `params: { forceType: 'ticket' }` → scanner-modal lit `useLocalSearchParams`, applique `lockedType`, et bloque l'auto-détection OCR de basculer vers 'invoice' si l'utilisateur est entré depuis Tickets. AUCUN ticket ne créera plus accidentellement une facture. (H) Backend smoke test (production): /api/optimizer/analyze 200, /api/email/parse 200, /api/scanner/ocr 200 (litellm warnings dans les logs sont des artefacts info; succès HTTP 200). VERIFICATION: tsc --noEmit → **0 erreur**. Metro Web bundle: **1795 modules SUCCESS** (+1 module errorSanitizer). EAS config unchanged: production=store, appVersionSource=local, runtimeVersion=fingerprint. FILES MODIFIED: app.json, app/scanner-modal.tsx (lockedType + useLocalSearchParams), app/more/email-import.tsx (normalize HEIC/PDF + humanizeError), app/more/documents.tsx (suppr Import IA banner), app/more/receipts.tsx (suppr Import IA banner + forceType ticket), src/services/featureFlags.ts (useIsPremium = hasPremiumAccess equivalent). FILES CREATED: src/lib/errorSanitizer.ts. SKIPPED to preserve credits (deferred): Famille premium UX collaborative refonte (avatars/dépenses partagées/qui doit combien — requires shared expense backend); add-receipt-manual/add-invoice manual forms enhancement; in-app 'Aide' tutorial button for Investments/Impôts (the empty state tutorial v3.7.13 already covers Investments); paywall premium/gratuit clarification copy refresh. RISKS REMAINING: None critical — build 54 fixes the 3 TestFlight P0 bugs (Voice paywall, HEIC import, technical errors visible)."
