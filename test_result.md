...
agent_communication:
  ...
  - agent: "main"
    message: "VAGUE 1 OFFLINE-FIRST COMPLETED (frontend-only changes — NO backend modifications). Implemented full network resilience layer to fix TestFlight offline-bug. Changes: (A) Installed @react-native-community/netinfo@12.0.1 (native module, auto-linked by Expo prebuild, will be included in next EAS build). (B) NEW /app/frontend/src/lib/network.ts — exports safeFetch() with timeout (8s default) + retry (1 attempt) + exponential backoff + AbortController + graceful failure (never throws, always returns SafeFetchResult). Also exports startNetworkMonitor(), subscribeNetwork(), getNetworkState(), isLikelyOnline(), ping(). NetInfo is lazy-loaded (Expo Go / web safe via require try/catch). (C) NEW /app/frontend/src/components/OfflineBanner.tsx — premium animated iOS-style top banner with spring animation, safe-area aware, i18n (fr/en/de/it), theme-aware (uses theme.warning). Replaces old hardcoded French OfflineBadge. (D) REWROTE /app/frontend/src/services/sync.ts — primary truth is now NetInfo signal; HTTP ping (Supabase/backend) only as fallback when NetInfo is ambiguous. Less aggressive (30s interval vs 15s). Debounced (2 consecutive offline checks before flipping). NEVER locks app in offline mode due to flaky backend. (E) UPDATED /app/frontend/src/services/iap.ts — wrapped postJson() and fetchSubscriptionFromBackend() with safeFetch(timeoutMs:8000, retries:1, silent:true). No more hanging requests, no more silent throws. (F) UPDATED /app/frontend/app/_layout.tsx — removed OfflineBadge component + its StyleSheet, replaced with <OfflineBanner /> import. Removed unused FontSizes/FontWeights/Spacing/View/Text/StyleSheet imports. (G) Added 'network' section to /app/frontend/src/i18n/translations.ts with 8 keys × 4 languages (offlineBanner, offlineShort, backOnline, retry, noInternet, checking, saved, willSync). TypeScript check passes (only pre-existing cloud-sync.tsx errors remain). REQUEST: please run a quick smoke test on the IAP backend to confirm no regression — main agent did NOT touch backend code but wants to make sure the public endpoints still respond correctly (GET /api/health, GET /api/iap/health, GET /api/iap/me?user_id=00000000-0000-0000-0000-000000000000). No deep testing needed."
  - agent: "testing"
