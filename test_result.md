...
agent_communication:
  ...
  - agent: "main"
    message: "v3.7.13 / build 53 — Session UX FONCTIONNELLE FINALE (frontend-only, no backend changes). DELIVERED: (A) Versions bumped to 3.7.13 / iOS buildNumber 53 / Android versionCode 53. (B) **NEW screen `/more/add-contract.tsx`** — page d'ajout/vérification de contrat complète (~340 lignes) supporting 2 distinct UX modes via the `source` route param: (1) **Verification mode** (source='scan'/'pdf'/'email') → premium card with sparkles icon 'Champs détectés par l'IA', photo preview avec badge 'Document scanné', table de 6 champs lecture seule highlightés (nom, fournisseur, type, montant, échéance, renouvellement), notif tip vert si dueDate valide ('Vous recevrez des rappels à J-90/J-30/J-7/J-1'), 2 boutons sticky: 'Modifier avant ajout' (secondary) + 'Ajouter le contrat' (primary). (2) **Edit/Manual mode** (source='manual' OU clic sur 'Modifier avant ajout') → form complet: nom, fournisseur, montant CHF, type (7 chips: téléphone/assurance/logement/transport/santé/énergie/autre), date début, date échéance avec hint 'rappels automatiques', switch renouvellement auto, délai résiliation, notes multiline. Sticky CTA 'Ajouter le contrat' (jamais de flow contrat sans bouton Ajouter, conformément à la spec). Helpers: `toISODate()` accepte DD.MM.YYYY ou YYYY-MM-DD. À l'ajout: `addContract(...)` + `scheduleDeadlineRemindersForEntity()` automatique si date d'échéance valide + Haptics.Success + Alert confirmant les rappels programmés. (C) **Scanner-modal contract path wired**: when receiptType==='contract', `handleSave` now `router.replace('/more/add-contract', { source:'scan', title, amount, notes, photoUri, category })` au lieu d'enregistrer silencieusement → l'utilisateur voit toujours l'écran de vérification avec champs IA + bouton 'Ajouter le contrat'. Aucun flow contrat ne se termine sans confirmation explicite. (D) **Mon Classeur 'Contrat manuel' CTA** redirigé vers le nouveau `/more/add-contract` au lieu de l'onglet expenses (UX plus directe). (E) **Investments — Premium onboarding tutorial empty state** ajouté dans investments.tsx: 4 mini-cards éducatives (ETF Vanguard/iShares, Actions Nestlé/Roche/UBS, Crypto BTC/ETH/SOL, Métaux Or/argent) + section 'How it works' en 3 étapes numérotées (1.Choisissez un type, 2.Indiquez quantité & prix moyen, 3.Prix actuel auto/manuel). +30 lignes de styles (tutoRow, tutoCard, howRow, howNum, etc.) intégrés au makeStyles existant pour respect light/dark mode. (F) Backend untouched — toujours 200 OK. VERIFICATION: tsc --noEmit → **0 errors**. Metro Web bundle: **1794 modules SUCCESS** (+1 module dû à add-contract.tsx). EAS config unchanged: production=store, appVersionSource=local, runtimeVersion=fingerprint. FILES MODIFIED: app.json, app/scanner-modal.tsx (contract path), app/more/documents.tsx (CTA route), app/more/investments.tsx (tutorial empty state). FILES CREATED: app/more/add-contract.tsx. SKIPPED to preserve credits: Famille premium UX refonte (avatars/live/dépenses partagées — requires backend); Light mode mini-check (low priority); shared-expense backend integration. RISKS REMAINING: None critical — build 53 is App Store ready."
