...
agent_communication:
  ...
  - agent: "testing"
    message: "v3.7.28 / build 68 — CLOUD SYNC E2E TEST (iteration_5) → PASS 5/5. Test cible : valider que la cloud sync Supabase fonctionne après application du SQL schema + autoSync subscriber + per-table column whitelist. (1) Login e2e-test@budgy.ch / Test1234! OK. (2) Transaction 'E2E-TEST-SYNC-V2 / 42.50 CHF / Courses' ajoutée et visible. (3) Log '[auto-sync] pushed 2 items' (prefs + transaction). (4) Transaction persiste après window.location.reload() + log '[bootstrap-sync] pulled 2 items from cloud'. (5) ZERO occurrence des codes 42501, 42P01, 42703, 23503, PGRST204, PGRST301. Le fix iter-5 dans cloudSync.ts (TABLE_FIELDS whitelist + TABLE_CONFLICT user_preferences:'user_id' + pickFields + safeUpsert avec conflictCol custom) règle définitivement les 2 régressions identifiées en iter-4 (42703 sur user_preferences PK + PGRST204 sur transactions.synced). Cloud Sync = OPÉRATIONNEL. Aucun retest nécessaire. Carry-over non-bloquants : 2 warnings RN-Web ('Unexpected text node' modal expenses + 'collapsable' attr non-boolean) — déjà documentés. Seed data en base : 1 transaction laissée pour e2e-test@budgy.ch."

  - agent: "main"
    message: "v3.7.28 / build 68 — FIX CLOUD SYNC COMPLET (cloudSync.ts uniquement). Bug fix de la régression critique 'data disappears after reconnect'. CHANGEMENTS dans /app/frontend/src/services/cloudSync.ts (1 seul fichier modifié) : (A) upsertTable() signature étendue avec param conflictCol (default 'id'). (B) Nouveau dict TABLE_FIELDS : whitelist stricte par table alignée sur docs/SUPABASE_SCHEMA.sql (14 tables, n'autorise que les colonnes réellement présentes en SQL). (C) Nouveau dict TABLE_CONFLICT = { user_preferences: 'user_id' } pour gérer les PK non-standard. (D) Helper pickFields(table, row) filtre les colonnes avant push. (E) Helper safeUpsert(table, rows) capture toute erreur per-table sans bloquer les tables suivantes (résout le bug 'failure on user_preferences cascade-aborts transactions push'). (F) Logs enrichis pour codes 42703 et PGRST204 (schema drift hints). Aucun bump version. tsc --noEmit = 0 erreur. testing_agent iteration_5 = PASS 5/5. Cloud Sync OPÉRATIONNEL en production sur Supabase https://supabase.budgy.ch. Le bug 'données qui disparaissent après reconnexion' est résolu. Version stable pour TestFlight 3.7.28 / build 68."
  - agent: "main"
  ...
  - agent: "main"
    message: "v3.7.29 / build 69 → APP STORE CANDIDATE FINALE. Phase 1 audit (12 parcours): 11/12 PASS. Phase 2 corrections : 4 fichiers modifiés. (1) /app/frontend/app/more/siri-assistant.tsx CRÉÉ : écran documentation Siri & Google Assistant avec deep-links budgy://quick-add, 5 exemples cliquables, wording honnête 'Compatible via Raccourcis iOS' (PAS de 'Siri natif'). (2) /app/frontend/app/(tabs)/more.tsx : +1 item menu 'Siri & Assistant' dans IA & Optimisation. (3) /app/frontend/src/lib/voiceLocalParser.ts : extension RECURRING_KEYWORDS avec 'contrat', 'police d\\'assurance', 'leasing'. (4) /app/backend/server.py line 1433 (_parse_voice_local sub_kw) : ajout 'contrat', 'leasing', 'police d\\'assurance', 'chaque mois', 'tous les mois', 'mensuel', 'par mois' pour le fallback regex backend (le frontend appelle /api/voice/parse, donc le fix backend était nécessaire en plus du frontend). (5) /app/frontend/app.json : version 3.7.28→3.7.29, buildNumber 68→69, versionCode 68→69. Phase 3 bump version OK. Phase 4 tests finaux iteration_8 : 4/4 PASS (Contrat→subscription/recurring/abonnement OK, Netflix régression OK, Migros pas de faux positif OK, Cloud Sync persistance OK). tsc --noEmit = 0 erreur. Audit URLs : 0 localhost/emergent dans le code prod (uniquement 1 commentaire dans network.ts). Verdict testing_agent : GO BUILD 69. Backend OpenAI key sk-emerg****d346 invalide → fallback regex actif (pré-existant, non bloquant)."
